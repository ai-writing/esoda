...
def is_adjacent(rr):
    ...
    for i in xrange(r_len):
        if r_list[i] == '*':
            if (i-1 >= 0 and not r_list[i-1].isalpha()) or (i+1 <= r_len-1 and not r_list[i+1].isalpha()):
                return True
    return False
